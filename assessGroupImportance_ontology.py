...
def readOntologyMapping(ontologyPath):
	toReturn = {}
	with open(ontologyPath) as f:
		data = f.readlines()
	for line in data[1:]:
		tokens = line.strip().split("\t")
		toReturn[tokens[0]] = set(tokens[1:])
	toReturn["Root"] = set()
	return toReturn

def createGraph(groupStatsFilePath, outputPath, threshold, ontologyMapping, mapping = None):
	...
	for i in range(len(groupArray)):
                ...
                componentsToCover = set(parent.components).copy()
	...
	listFile.write("node 1\tnode 2\tstatus\tfraction of shared terms\tshared terms\n")
	queue = [root]
	seen = set()
	pairSet = set ()
	while queue:
...
if __name__ == "__main__":
	...
	mapping = None
	if len(sys.argv)>5:
		mapping = readMapping(sys.argv[5])
	threshold = float(sys.argv[3])
	ontologyPath = sys.argv[4]
