	...
	for line in data[1:]:
		...
		for i in range(len(groupFactors)-1):
			...
			for j in range(i+1, len(groupFactors)):
				...
				scoreMatrix[ind1][ind2] += 1.0
				scoreMatrix[ind2][ind1] += 1.0
	# convert all values to log
	scoreMatrix = np.log(scoreMatrix+1)

	fig, ax = plt.subplots()
	img = ax.imshow(scoreMatrix, cmap=cm.Greens, interpolation="none") 
	fig.colorbar(img)
	plt.title("Log Factor Co-occurrence in a Group")
        #factors.reverse()
